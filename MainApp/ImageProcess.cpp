...
uint64_t ImageProcess::GetMemoryUsage() {
	std::lock_guard<std::mutex> lock(memoryMutex);
	PROCESS_MEMORY_COUNTERS_EX pmc;
	if (!GetProcessMemoryInfo(GetCurrentProcess(), (PROCESS_MEMORY_COUNTERS*)&pmc, sizeof(pmc))) {
		fileLogger->warn("GetProcessMemoryInfo failed!");
		return 0;
	}
...
void ImageProcess::opencvBlurImage() {
	uint64_t memoryBeforeOpencv = this->GetMemoryUsage();
	int64_t startTime = cv::getTickCount();
	OpenCV::ImageBlur(&srcImage, &dstImageOpenCV, kernelSize);
	int64_t endTime = cv::getTickCount();
	uint64_t memoryAfterOpencv = this->GetMemoryUsage();
...
void ImageProcess::makeReport() {
	...
	fs << "index:" << index << " Custom::ImageBlur memory usage: " << memoryUsageCustom / 1024.0 << " KB" << std::endl;
	fs << "index:" << index << " OpenCV::ImageBlur memory usage: " << memoryUsageOpenCV / 1024.0 << " KB" << std::endl;
...
void ImageProcess::saveImages(const std::string& resultPath) {
	...
	cv::imwrite(resultPath + "/" + std::to_string(index) + "_opencv_blur.png", dstImageOpenCV.toMat());
	cv::imwrite(resultPath + "/" + std::to_string(index) + "_custom_blur.png", dstImage.toMat());
